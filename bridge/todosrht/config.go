...
const credTypeText = `
TODOSRHT has recently altered it's authentication strategies. Servers deployed
...
passes the session cookie with each request. TODOSRHT Cloud and servers deployed
...
func (j *TodoSourceHut) Configure(repo *cache.RepoCache, params core.BridgeParams, interactive bool) (core.Configuration, error) {
	...
	if project == "" {
		if !interactive {
			return nil, fmt.Errorf("Non-interactive-mode is active. Please specify the TODOSRHT project key via the --project option.")
		}
		project, err = input.Prompt("TODOSRHT project key", "project", input.Required)
		if err != nil {
			return nil, err
		}
	}
