...
// User the JSON object representing a TODOSRHT user
// https://docs.atlassian.com/software/todosrht/docs/api/REST/8.2.6/#api/2/user
type User struct {
...
// comments
// https://docs.atlassian.com/software/todosrht/docs/api/REST/8.2.6/#api/2/issue-getComments
type Comment struct {
	ID           string `json:"id"`
...
// https://docs.atlassian.com/software/todosrht/docs/api/REST/8.2.6/#api/2/issue-getIssue
...
func (clp *ChangeLogPage) IsLastPage() bool {
	// NOTE(josh): The "isLast" field is returned on TODOSRHT cloud, but not on
	// TODOSRHT server. If we can distinguish which one we are working with, we can
...
// https://docs.atlassian.com/software/todosrht/docs/api/REST/8.2.6/#api/2/search
...
// another Status in a TODOSRHT workflow
...
// Client Thin wrapper around the http.Client providing todosrht-specific methods
// for API endpoints
type Client struct {
	*http.Client
	serverURL string
...
func (client *Client) Login(credType, login, password string) error {
	...
	default:
		panic("unknown todo.sr.ht cred type")
	}
}
...
// https://docs.atlassian.com/software/todosrht/docs/api/REST/8.2.6/#api/2/issue
...
// endpoint
// https://docs.atlassian.com/software/todosrht/docs/api/REST/8.2.6/#api/2/issue-getComment
...
// /issue/{IssueIdOrKey}/changelog endpoint (for TODOSRHT cloud) or
...
func (client *Client) GetChangeLog(idOrKey string, maxResults int, startAt int) (*ChangeLogPage, error) {
	...
	if response.StatusCode == http.StatusNotFound {
		// The issue/{IssueIdOrKey}/changelog endpoint is only available on TODOSRHT cloud
		// products, not on TODOSRHT server. In order to get the information we have to
	...
	// TODOSRHT cloud returns changelog entries in the "values" list, whereas TODOSRHT
...
// CreateIssue creates a new TODOSRHT issue and returns it
...
// UpdateIssueTitle changes the "summary" field of a TODOSRHT issue
...
// UpdateIssueBody changes the "description" field of a TODOSRHT issue
