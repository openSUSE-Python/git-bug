...
// Init .
func (ji *todosrhtImporter) Init(ctx context.Context, repo *cache.RepoCache, conf core.Configuration) error {
	ji.conf = conf
			...
			for changelogIter =
				...
				for ; opIdx < len(snapshot.Operations); opIdx++ {
					exportTimeStr, hasTime := snapshot.Operations[opIdx].GetMetadata(
						metaKeyTodoSourceHutExportTime)
					if !hasTime {
						continue
					}
...
func (ji *todosrhtImporter) ensurePerson(repo *cache.RepoCache, user User) (*cache.IdentityCache, error) {
	// Look first in the cache
	i, err := repo.Identities().ResolveIdentityImmutableMetadata(
		metaKeyTodoSourceHutUser, string(user.Key))
	if err == nil {
		return i, nil
	}
	...
	i, err = repo.Identities().NewRaw(
		...
		nil,
		map[string]string{
			metaKeyTodoSourceHutUser: user.Key,
		},
	)
...
// Create a bug.Bug based from a TODOSRHT issue
func (ji *todosrhtImporter) ensureIssue(repo *cache.RepoCache, issue Issue) (*cache.BugCache, error) {
	...
	b, err := repo.Bugs().ResolveMatcher(func(excerpt *cache.BugExcerpt) bool {
		...
		return excerpt.CreateMetadata[core.MetaKeyOrigin] == target &&
			excerpt.CreateMetadata[metaKeyTodoSourceHutId] == issue.ID &&
			excerpt.CreateMetadata[metaKeyTodoSourceHutProject] == ji.conf[confKeyProject]
	})
	...
	if entity.IsErrNotFound(err) {
		b, _, err = repo.Bugs().NewRaw(
			...
			map[string]string{
				...
				metaKeyTodoSourceHutId:      issue.ID,
				metaKeyTodoSourceHutKey:     issue.Key,
				metaKeyTodoSourceHutProject: ji.conf[confKeyProject],
...
// Return a unique string derived from a unique todosrht id and a timestamp
func getTimeDerivedID(todosrhtID string, timestamp Time) string {
	return fmt.Sprintf("%s-%d", todosrhtID, timestamp.Unix())
}

// Create a bug.Comment from a TODOSRHT comment
func (ji *todosrhtImporter) ensureComment(repo *cache.RepoCache, b *cache.BugCache, item Comment) error {
	// ensure person
	...
	targetOpID, err := b.ResolveOperationWithMetadata(metaKeyTodoSourceHutId, item.ID)
	...
	if targetOpID == "" && err == cache.ErrNoMatchingOp {
		...
		commentId, op, err := b.AddCommentRaw(
			...
			nil,
			map[string]string{
				metaKeyTodoSourceHutId: item.ID,
			},
		)
		if err != nil {
			return err
	...
	_, err = b.ResolveOperationWithMetadata(metaKeyTodoSourceHutId, derivedID)
	if err == nil {
	...
	_, err = b.EditCommentRaw(
		...
		map[string]string{
			metaKeyTodoSourceHutId: derivedID,
		},
	)
...
// Return a unique string derived from a unique todosrht id and an index into the
// data referred to by that todosrht id.
func getIndexDerivedID(todosrhtID string, idx int) string {
	return fmt.Sprintf("%s-%d", todosrhtID, idx)
}

func labelSetsMatch(todosrhtSet []string, gitbugSet []common.Label) bool {
	if len(todosrhtSet) != len(gitbugSet) {
		return false
	}

	sort.Strings(todosrhtSet)
	...
	for idx, value := range todosrhtSet {
...
// Create a bug.Operation (or a series of operations) from a TODOSRHT changelog
// entry
func (ji *todosrhtImporter) ensureChange(repo *cache.RepoCache, b *cache.BugCache, entry ChangeLogEntry, potentialOp dag.Operation) error {
	...
	_, err := b.ResolveOperationWithMetadata(metaKeyTodoSourceHutDerivedId, entry.ID)
	if err == nil {
		return nil
	...
	// TODOSRHT. For example, when an issue is closed both its "status" and its
	...
	// number of fields in one go, so we break up the single TODOSRHT changelog
	...
	for _, item := range entry.Items {
		...
		case "labels":
			...
			if isRightType && labelSetsMatch(addedLabels, opr.Added) && labelSetsMatch(removedLabels, opr.Removed) {
				_, err := b.SetMetadata(opr.Id(), map[string]string{
					metaKeyTodoSourceHutDerivedId: entry.ID,
				})
				if err != nil {
					return err
				}
		...
		case "summary":
			// NOTE(josh): TODOSRHT calls it "summary", which sounds more like the body
		...
		case "description":
			// NOTE(josh): TODOSRHT calls it "description", which sounds more like the
	...
	for idx, item := range entry.Items {
		...
		_, err := b.ResolveOperationWithMetadata(metaKeyTodoSourceHutDerivedId, derivedID)
		if err == nil {
			continue
		}
		if err != cache.ErrNoMatchingOp {
		...
		case "labels":
			...
			op, err := b.ForceChangeLabelsRaw(
				...
				map[string]string{
					metaKeyTodoSourceHutId:        entry.ID,
					metaKeyTodoSourceHutDerivedId: derivedID,
				},
			)
			if err != nil {
				return err
		...
		case "status":
			...
			if hasMap {
				...
				ji.out <- core.NewImportError(
					fmt.Errorf(
						"No git-bug status mapped for todosrht status %s (%s)",
